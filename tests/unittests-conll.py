...
def read(key, response):
    ...
    reader.get_coref_infos('tests-conll/%s' % key, 'tests-conll/%s' % response)
...
def test_B1():
    ...
    assert evaluate(doc, lea) == approx([(2 * 0 + 3 / 3) / 5, (3 * 0 + 2) / 5,
                                         2 * 1 / 5 * 2 / 5 / (1 / 5 + 2 / 5)])
    assert evaluate(doc, [blancc, blancn]) == approx(
        [1 / 2 * (1 / 4 + 1 / 3), 1 / 2 * (1 / 4 + 1 / 3), 1 / 2 * (1 / 4 + 1 / 3)])
...
def test_C1():
    ...
    assert evaluate(doc, lea) == approx([(2 * 0 + 3 / 3 + 2) / 7,
                                         ...
                                         2 * 3 / 7 * 4 / 7 / (3 / 7 + 4 / 7)])
    assert evaluate(doc, [blancc, blancn]) == approx(
        [1 / 2 * (2 / 5 + 10 / 16), 1 / 2 * (2 / 5 + 10 / 16), 1 / 2 * (2 / 5 + 10 / 16)])
